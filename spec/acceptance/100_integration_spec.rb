...
describe 'Atlassian Bamboo with PostgreSQL 9.3 Database' do
  ...
  include_examples 'an acceptable Atlassian Bamboo instance', 'using a PostgreSQL database' do
    before :all do
      ...
      if ENV['CIRCLECI']
        `docker run --link "#{@container_db.id}:db" postgres:9.3 psql --host "db" --user "postgres" --command "create database bamboodb owner postgres encoding 'utf8';"`
      else
...
describe 'Atlassian Bamboo with MySQL 5.6 Database' do
  ...
  include_examples 'an acceptable Atlassian Bamboo instance', 'using a MySQL database' do
    before :all do
      ...
      # Create JIRA database
      if ENV['CIRCLECI']
        `docker run --link "#{@container_db.id}:db" mysql:5.6 mysql --host "db" --user=root --password=mysecretpassword --execute 'CREATE DATABASE bamboodb CHARACTER SET utf8 COLLATE utf8_bin;'`
      else
